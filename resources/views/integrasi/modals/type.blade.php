<div id="type{{ $item['id'] }}" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <div class="p-4">
                <form action="{{ route('integrasi.update.type', $item['id']) }}" method="post">
                    @csrf
                    <div class="flex flex-row gap-1 mb-2">
                        <select name="type" required
                            ...
                            <option {{ $item['type'] == 'auth' ? 'selected' : '' }} value="auth">Autentikasi Dasar
                            </option>
                            <option {{ $item['type'] == 'token' ? 'selected' : '' }} value="token">Token</option>
                            <option {{ $item['type'] == 'default' ? 'selected' : '' }} value="default">Tanpa Autentikasi
                            </option>
                        </select>
                    </div>
